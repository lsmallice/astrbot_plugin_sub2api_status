...
- QQ 活动默认关闭，并且只响应配置的群号；活动启用后每个 QQ 发送者和每个 Sub2API 用户各只能成功领取一次。
- 使用 `/绑定` 生成短期网页登录链接；网页登录后还必须由同一个 QQ 发送 `/绑定确认 6位验证码` 才会真正绑定，防止群内链接被他人复制后抢绑。
...
- 领取状态保存于 AstrBot 插件自己的 SQLite 文件，不修改 Sub2API 数据库；余额增加使用官方 `Idempotency-Key`，重试不会重复入账。
...
| `gift_enabled` | `false` | 是否启用 QQ 群赠送余额 |
| `gift_amount` | `10` | 每次赠送额度，范围 `0.01..1000` |
| `gift_notes` | `QQ群活动赠送余额` | 写入官方余额流水的备注 |
...
| `gift_allowed_group_ids` | `123456789` | 允许参与的 QQ 群号，多个用逗号分隔；留空不响应 |
| `claim_registration_window_hours` | `24` | 仅允许注册后指定小时数内的 Sub2API 账号领取；`0` 表示不限制 |
...
管理员 Key 拥有完整管理权限。请限制 AstrBot 配置文件权限，不要把 Key 放在聊天消息、截图、日志或公开仓库中。
...
打开机器人返回的链接，在主站登录自己的账号并点击确认；再回到发起绑定的 QQ 发送页面显示的 `/绑定确认 6位验证码`。最终绑定成功后，在允许的 QQ 群中发送：

```text
@机器人 领取
```

也可以发送 `/绑定状态` 查看当前绑定状态。领取不再接受任意邮箱或用户名，避免他人冒用账号；领取失败不会消耗资格，管理员可修复配置或服务后重试。用户侧不会看到管理员 Key、用户余额或具体内部错误。

当 `claim_registration_window_hours` 大于 `0` 时，领取前会查询官方账号的 `created_at`，
只有注册时间距当前不超过配置小时数的账号可以领取。该限制在插件侧校验，超时账号不会预留领取记录，
也不会调用余额增加接口。
